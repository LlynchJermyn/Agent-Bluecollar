...
// RAM Cleanup Funktion (Verhindert Error 137 bei mehreren Instanzen)
...
async function processAndSendWebhook(store_id, address, lat, lon, webhook_url, broadband_type) {
    ...
    try {
        // JITTER: Zufällige Verzögerung (0-5 Sek), um Akamai auszutricksen, wenn 3 Agenten gleichzeitig starten
        const jitterDelay = Math.floor(Math.random() * 5000);
        console.log(`\n--- [START] Store ${store_id}: Jitter-Verzögerung ${jitterDelay}ms ---`);
        ...
        browser = await puppeteer.launch({
            ...
            args: [
                ...
                '--js-flags="--max-old-space-size=1024"' // RAM Limit pro Tab
            ],
        ...
        // REDUZIERTE WARTEZEIT 1: Von 15s auf 7s
        try {
        ...
        if (!tableLoaded) {
            ...
            // REDUZIERTE WARTEZEIT 2: Von 40s auf 20s
            await page.reload({ waitUntil: 'domcontentloaded', timeout: 20000 }).catch(() => {});
            resolvedUrl = page.url();
            await humanizeMouse(page);
            
            // REDUZIERTE WARTEZEIT 3: Von 35s auf 15s
            try {
                await page.waitForFunction(() => {
                    const table = document.querySelector('.table-responsive table');
                    return table && table.tBodies[0] && table.tBodies[0].rows.length > 0;
        ...
        killChromeProcesses(); // Hält den RAM sauber!
        agentLock.unlock();
    }
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Agent Bluecollar (Speed & Jitter Edition) läuft auf Port ${PORT}`);
});