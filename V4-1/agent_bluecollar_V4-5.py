# ----------------------------------------
# AGENT BLUECOLLAR V4.5: PARALLEL & DEBUG EDITION
# Basis: V4.5 Native Linux (Source 7) mit resolved_url Fix & Robust Table Detection
# ----------------------------------------
...
async def process_and_send_webhook(store_id: float, address: str, lat: float, lon: float, webhook_url: str, broadband_type: str):
    ...
    resolved_url = "N/A" # Fix: Initialisierung um NameError zu vermeiden
    ...
    async with agent_lock:
        ...
        async with Stealth().use_async(async_playwright()) as p:
            try:
                ...
                if broadband_type == "fixed":
                    logger.info(f"[{store_id}] --- [PHASE 1] Suche ---")
                    ...
                    resolved_url = page.url # Debug-URL nach Redirect abgreifen
                    ...
                    logger.info(f"[{store_id}] --- [PHASE 1] MOBILE Direktzugriff ---")
                    await page.goto(resolved_url, wait_until="domcontentloaded", timeout=40000)

                # ==========================================
                # OPTIMIERTES POLLING (Robust gegen URL-Filter)
                # ==========================================
                logger.info(f"[{store_id}] ⏳ Warte auf Daten...")
                try:
                    await page.wait_for_function("""
                        () => {
                            const tables = document.querySelectorAll('table');
                            // Extrem tolerante Tabellen-Suche (Provider, Technology, Mbps oder Holding Company)
                            const hasTable = Array.from(tables).some(t => 
                                t.rows.length > 0 && 
                                (t.innerText.includes('Provider') || t.innerText.includes('Technology') || t.innerText.includes('Mbps') || t.innerText.includes('Holding Company'))
                            );
                            
                            const bodyText = document.body.innerText;
                            const noData = bodyText.includes('Not Served') || 
                                           bodyText.includes('0 Providers') || 
                                           bodyText.includes('No data available') ||
                                           bodyText.includes('Unserved');
                    ...
                    markdown_table = await page.evaluate("""(type) => {
                        ...
                        // Wir nehmen einfach die Tabelle mit den meisten Reihen, egal wie der Header heißt
                        let target = tables.reduce((prev, curr) => (prev.rows.length > curr.rows.length) ? prev : curr, {rows: []});
                        
                        if (!target || !target.rows || target.rows.length <= 1) return "EMPTY";
                        
                        let md = "";
                        for (let i = 0; i < target.rows.length; i++) {
                            let rowData = Array.from(target.rows[i].cells).map(c => c.innerText.replace(/\\n/g, ' ').trim());
                            
                            // Bereinigung falls "Holding Company" in der ersten Spalte hängt
                    ...
                    logger.warning(f"[{store_id}] ❌ Timeout/Keine Daten gefunden. {wait_err}")
                    markdown_table = "EMPTY"
                
                await browser.close()
                duration = round(time.time() - start_time, 2)
                
                # Payload enthält garantiert broadband_type und resolved_url
                final_payload = {
                    "store_id": store_id,
                    "address": address,
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.info(f"[{store_id}] ✅ Webhook übermittelt.")
